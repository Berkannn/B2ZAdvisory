...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "guides.registryAcd" });

  return {
    title: t("seoTitle"),
    description: t("seoDescription"),
    alternates: buildAlternates("/cbam-kayit-sistemi-acd-basvuru", locale),
    openGraph: {
      title: t("seoTitle"),
      description: t("seoDescription"),
      locale,
...
export default async function RegistryAcdGuidePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("guides.registryAcd");
        ...
        <div className="container-page max-w-3xl">
          <p className="text-sm leading-relaxed text-carbon-700 sm:text-base">
            {t.rich("relatedParagraph", {
              link: (chunks) => (
                <Link href="/ab-ithalatcisi-tedarikci-beklentileri" className={linkClass}>
                  {chunks}
                </Link>
              ),
            })}
