...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "guides.cement" });

  return {
    title: t("seoTitle"),
    description: t("seoDescription"),
    alternates: buildAlternates("/cimento-sektorunde-cbam", locale),
    openGraph: {
      title: t("seoTitle"),
      description: t("seoDescription"),
      locale,
...
export default async function CementGuidePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("guides.cement");
        ...
        <div className="container-page max-w-3xl">
          <p className="text-sm leading-relaxed text-carbon-700 sm:text-base">
            {t.rich("relatedParagraph", {
              ...
              sectorLink: (chunks) => (
                <Link href="/cbam/cimento" className={linkClass}>
                  {chunks}
                </Link>
              ),
            })}
