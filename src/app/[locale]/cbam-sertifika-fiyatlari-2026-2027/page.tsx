...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "guides.certificatePrices" });

  return {
    title: t("seoTitle"),
    description: t("seoDescription"),
    alternates: buildAlternates("/cbam-sertifika-fiyatlari-2026-2027", locale),
    openGraph: {
      title: t("seoTitle"),
      description: t("seoDescription"),
      locale,
...
export default async function CertificatePricesGuidePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("guides.certificatePrices");
